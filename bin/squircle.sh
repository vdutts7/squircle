...
#   marker path = "${OUTPUT}${CLIP_MARKER_SUFFIX}"; clipped → : > marker ; else /bin/unlink marker.
...
cleanup() {
  ...
  [[ -n "$MASK_TMP" && -f "$MASK_TMP" ]] && /bin/unlink "$MASK_TMP"

  [[ -n "$TMP_PNG" && -f "$TMP_PNG" ]] && /bin/unlink "$TMP_PNG"

  [[ -n "$TMP_SVG_PNG" && -f "$TMP_SVG_PNG" ]] && /bin/unlink "$TMP_SVG_PNG"

  [[ -n "$TMP_QL_DIR" && -d "$TMP_QL_DIR" ]] && /usr/bin/trash "$TMP_QL_DIR" 2>/dev/null

  [[ -n "$CLIP_FRAME_TMP" && -f "$CLIP_FRAME_TMP" ]] && /bin/unlink "$CLIP_FRAME_TMP"

  [[ -n "$CLIP_MASKED_TMP" && -f "$CLIP_MASKED_TMP" ]] && /bin/unlink "$CLIP_MASKED_TMP"

  [[ -n "$CLIP_DIFF_TMP" && -f "$CLIP_DIFF_TMP" ]] && /bin/unlink "$CLIP_DIFF_TMP"

  [[ -n "$CLIP_FRAME_FLAT_TMP" && -f "$CLIP_FRAME_FLAT_TMP" ]] && /bin/unlink "$CLIP_FRAME_FLAT_TMP"

  [[ -n "$CLIP_MASKED_FLAT_TMP" && -f "$CLIP_MASKED_FLAT_TMP" ]] && /bin/unlink "$CLIP_MASKED_FLAT_TMP"

  # Belt-and-suspenders: known suffixes only. Never call `/bin/unlink` with zero operands (zsh null_glob + no
  ...
  if [[ -n "${SQUIRCLE_TMPBASE:-}" ]]; then
    ...
    if [[ -d "${SQUIRCLE_TMPBASE}.ql.d" ]]; then
      ...
      /usr/bin/trash "${SQUIRCLE_TMPBASE}.ql.d" 2>/dev/null
  ...
  if [[ -n "${DOMAIN_RIP_TMP:-}" && -d "${DOMAIN_RIP_TMP}" ]]; then
    ...
    /usr/bin/trash "${DOMAIN_RIP_TMP}" 2>/dev/null
...
apply_plate_fx() {
  ...
  /bin/unlink "$tmp" "$tmp2" 2>/dev/null || true
...
main() {
  ...
  if (( CLIP_DETECT == 1 )); then
    ...
    if [[ "$BG_HEX" == "OPAQUE" ]]; then
      ...
      [[ -n "$OUTPUT" ]] && /bin/unlink "${OUTPUT}${CLIP_MARKER_SUFFIX}" 2>/dev/null || true
